...
В основе лежит `Lagnuty/tg-api-zapret` в папке `core`: приложение использует его Telethon-слой для входа в пользовательский Telegram-аккаунт, SOCKS5H proxy и чтение сообщений. Текущая версия приложения: `0.1.6`, ядра: `0.4.30`.
...
```text
dist\msi\AvtoVinchickTG-0.1.6.msi
```
...
```text
AvtoVinchickTG-0.1.6.msi
```

Tag релиза должен быть вида `v0.1.6` или `0.1.6`. После скачивания приложение закроется, запустит `msiexec` в silent-режиме и перезапустится.
