...
В основе лежит `Lagnuty/tg-api-zapret` в папке `core`: приложение использует его Telethon-слой для входа в пользовательский Telegram-аккаунт, SOCKS5H proxy и чтение сообщений. Текущая версия приложения: `0.1.14`, ядра: `0.4.30`.
...
```text
dist\msi\AvtoVinchickTG-0.1.14.msi
```
...
```text
AvtoVinchickTG-0.1.14.msi
```

Tag релиза должен быть вида `v0.1.14` или `0.1.14`. После скачивания приложение закроется, запустит `msiexec` в silent-режиме и перезапустится.
