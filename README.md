...
- Локальная web-панель, которая открывается в браузере, и tray-меню с открытием страницы, статусом, перезапуском и выходом.
- Настройки телефона, bot token, `chat_id`, исходного чата и proxy.
...
- Постоянный журнал виден на всех страницах; логи также пишутся в `data/logs/app.log` рядом с exe.
- Анкеты из альбомов Дайвинчика собираются в одно уведомление с текстом и до трех фото.
- Анкеты без описания, включая варианты только с именем, городом, возрастом и фото, сразу дизлайкаются командой `3`.
