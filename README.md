...
В основе лежит `Lagnuty/tg-api-zapret` в папке `core`: приложение использует его Telethon-слой для входа в пользовательский Telegram-аккаунт, SOCKS5H proxy и чтение сообщений. Текущая версия приложения: `0.1.21`, ядра: `0.4.30`.
...
```text
dist\msi\AvtoVinchickTG-0.1.21.msi
```
...
```text
AvtoVinchickTG-0.1.21.msi
```

Tag релиза лучше делать вида `v0.1.21` или `0.1.21`, но автообновление также умеет tag вроде `release`, если версия есть в имени MSI: `AvtoVinchickTG-0.1.21.msi`. После скачивания приложение закроется, запустит `msiexec` в silent-режиме и перезапустится.
