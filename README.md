...
В основе лежит `Lagnuty/tg-api-zapret` в папке `core`: приложение использует его Telethon-слой для входа в пользовательский Telegram-аккаунт, SOCKS5H proxy и чтение сообщений. Текущая версия приложения: `0.1.2`, ядра: `0.4.30`.
...
- Автопроверка обновлений приложения через GitHub Releases. Если вышла новая версия, появится кнопка `Доступно обновление v...`.
...
Для релиза соберите exe и загрузите zip-архив с содержимым папки:

```text
dist\AvtoVinchickTG
```

Имя asset лучше делать таким:

```text
AvtoVinchickTG-0.1.2.zip
```

Tag релиза должен быть вида `v0.1.2` или `0.1.2`. После скачивания приложение распакует архив, заменит файлы рядом с текущим exe и перезапустится.
